...
import com.mttk.orche.service.EventDataService;

@RestController
@RequestMapping(value = "/eventData")
public class EventDataController extends PersistableControllerBase {
	@Override
	public Class getServiceClass() {
		return EventDataService.class;
	}
	...
	public Document queryByCriteria(String criteria, Pageable pageable, HttpServletRequest request,
		...
		EventDataService eventDataService = ServerLocator.getServer().getService(EventDataService.class);
		...
		if (pageable == null || pageable.getSize() <= 0) {
			// 不分�?
			List<Document> list = eventDataService.find(histName, filter, 0, 0, sort);
			...
			if (pageable.getTotal() <= 0) {
				pageable.setTotal((int) eventDataService.count(histName, filter));
			}
			//
			if (pageable.getTotal() > 0) {
				list = eventDataService.find(histName, filter, (pageable.getPage() - 1) * pageable.getSize(),
	...
	public Document histList() throws Exception {
		EventDataService service = ServerLocator.getServer().getService(EventDataService.class);
