...
export const sendReplayInputInternal = async (agentRuntime: AgentRuntime) => {
    agentRuntime.status.value = ExecutionStatus.EXECUTING
    try {

        // 构建SSE URL
        const sseUrl = `${config.apiBaseURL}/chat/replay/${agentRuntime.agentId.value}/${agentRuntime.sessionId.value}`
        // 使用SSE工具类创建连接
